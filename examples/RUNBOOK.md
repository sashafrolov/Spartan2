...
## Zaratan flow for non-streaming KZG: 
Build: `RUSTFLAGS="-C target-cpu=native" cargo build --example neutronnova_hyperkzg_video_editing --release -j 96`
Reserve machine: `srun --partition=standard --nodes=1 --ntasks=1 --cpus-per-task=64 --mem=256G --time=2:00:00 --pty bash`
Run: `RUST_LOG=neutronnova_hyperkzg_video_editing=info,spartan2::neutronnova_zk_ram_optimized=info RUSTFLAGS="-C target-cpu=native" cargo run --example neutronnova_hyperkzg_video_editing --release`
To get RAM usage: `RUST_LOG=neutronnova_hyperkzg_video_editing=info,spartan2::neutronnova_zk_ram_optimized=info RUSTFLAGS="-C target-cpu=native" /usr/bin/time -v cargo run --example neutronnova_hyperkzg_video_editing --release`
