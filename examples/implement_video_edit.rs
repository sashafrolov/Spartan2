...
mod utils;
use utils::{
  GBLUR_RADIUS, SIGMA, create_gblur_matrix_u64, create_resizing_sparse, dense_sparse_matmul_u64,
  matrix_matrix_product_band_left_u64, matrix_matrix_product_band_right_u64, read_color_png,
...
fn split_channels(image: &[Vec<(u8, u8, u8)>]) -> (Vec<Vec<u64>>, Vec<Vec<u64>>, Vec<Vec<u64>>) {
  let r = image
    .iter()
    .map(|row| row.iter().map(|&(r, _, _)| r as u64).collect())
    .collect();
  let g = image
    .iter()
    .map(|row| row.iter().map(|&(_, g, _)| g as u64).collect())
    .collect();
  let b = image
    .iter()
    .map(|row| row.iter().map(|&(_, _, b)| b as u64).collect())
    .collect();
  (r, g, b)
}
...
fn apply_resize_channel(channel: &[Vec<u64>]) -> Vec<Vec<u8>> {
  ...
  assert!(
    ...
    "image dimensions must be even for 2x downscaling"
  );
