// NeutronNova 2x downscaling (streaming). Folds proofs for each channel (R/G/B) of each frame.
//
// Run with:
//   RUST_LOG=neutron_nova_streaming_resizing=info,spartan2::neutronnova_zk_streaming=info RUSTFLAGS="-C target-cpu=native" cargo run --example neutron_nova_streaming_resizing --release
...
fn main() {
  ...
  info!(
    ...
    image_height = IMAGE_DIMS.0,
    image_width = IMAGE_DIMS.1,
    "starting NeutronNova streaming resizing benchmark"
  );
  ...
  let frame_offset: u64 = env::var("SNARK_EDITING_FRAME_OFFSET")
    ...
    .unwrap_or(0);
  let t0 = Instant::now();
  let step_circuits: Vec<ResizingCircuit<<E as Engine>::Scalar>> = (0..NUM_CIRCUITS)
    .into_par_iter()
    .map(|i| {
      let frame_idx = (i / 3) as u64 + 1 + frame_offset;
      let channel = CHANNELS[i % 3];
      ResizingCircuit::<<E as Engine>::Scalar>::new(&channel_path_format, channel, frame_idx)
    })
    .collect();
