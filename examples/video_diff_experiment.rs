...
fn main() {
    ...
    for entry in &frame_entries {
        ...
        assert_eq!(source_image.len(), ffmpeg_image.len(), "height mismatch for {filename_str}");
        assert_eq!(source_image[0].len(), ffmpeg_image[0].len(), "width mismatch for {filename_str}");
        assert_eq!(source_image.len(), jnd_map.len(), "jnd height mismatch for {filename_str}");
        assert_eq!(source_image[0].len(), jnd_map[0].len(), "jnd width mismatch for {filename_str}");
        ...
        diff_pixels
            ...
            .zip(source_image.par_iter().zip(ffmpeg_image.par_iter()).zip(jnd_map.par_iter()))
            .for_each(|(chunk, ((source_row, ffmpeg_row), jnd_row))| {
                for (out, ((&s, &f), &jnd)) in chunk.iter_mut().zip(source_row.iter().zip(ffmpeg_row.iter()).zip(jnd_row.iter())) {
