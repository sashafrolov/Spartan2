...
impl<T: SerializeRaw + DeserializeRaw> FileVec<T> {
        ...
        T: Send + Sync + Debug,
    {
        ...
        if more_than_one_batch || batch_is_larger_than_buffer {
            ...
            file.truncate(offset + actual as u64).expect("failed to truncate file");
        ...
        B: SerializeRaw + DeserializeRaw + Send + Sync,
    {
        ...
        if more_than_one_batch {
            ...
            file_1.truncate(offset_1 + actual_1 as u64).expect("failed to truncate file");
            file_2.truncate(offset_2 + actual_2 as u64).expect("failed to truncate file");
        ...
        C: SerializeRaw + DeserializeRaw + Send + Sync,
    {
        ...
        if more_than_one_batch {
            ...
            file_1.truncate(offset_1 + actual_1 as u64).expect("failed to truncate file");
            file_2.truncate(offset_2 + actual_2 as u64).expect("failed to truncate file");
            file_3.truncate(offset_3 + actual_3 as u64).expect("failed to truncate file");
            file_1.rewind().expect("failed to seek file");
            file_2.rewind().expect("failed to seek file");
